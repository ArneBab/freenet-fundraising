...
- Freund-zu-Freund Darknet (mit optionalem Opennet)
- Nutzungsabhängige Lebenszeit
...
#+BEGIN_QUOTE
/The practical cool result now, is that the Mempo repository can not be censored, DDoSed or taken offline, despite having just 1 tiny server (or no server at all)/ — rfreeman in the article apt-get over Freenet.
#+END_QUOTE
