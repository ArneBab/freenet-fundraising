...
#+BEGIN_SRC python :tangle routingsim.py :exports none :results output
  ...
  def smallworldbydistance(locs, start, target, filepath=None):
      path = [start]
      # small world routing
      smallworldnet = {}
  ...
  def smallworldbyreject(locs, start, target, filepath=None):
      path = [start]
      # small world routing
      smallworldnet = {}
      sortedlocs = sorted(list(locs))
      ...
      def chooselink():
            for i in range(100):
              ...
              while potentialpeer in smallworldnet[loc] or potentialpeer == loc:
                potentialpeer = random.choice(sortedlocs)
              d = min(abs(potentialpeer - loc), abs(loc - potentialpeer))
              if not reject(smallworldnet[loc], d):
  ...
  def smallworldbyindex(locs, start, target, filepath=None):
      path = [start]
      # small world routing
      smallworldnet = {}
      sortedlocs = sorted(list(locs))
  ...
  def smallworldapprox(locs, start, target, filepath=None):
      path = [start]
      # small world routing
      smallworldnet = {}
      sortedlocs = sorted(list(locs))
  ...
  for i in range(10):
      ...
      smallworldnet, smallworldpath = smallworldapprox(locs, start, target)
      smallworldnetindex, smallworldpathindex = smallworldbyindex(locs, start, target)
      smallworldnetreject, smallworldpathreject = smallworldbyreject(locs, start, target)
      smallworldnetdistance, smallworldpathdistance = smallworldbydistance(locs, start, target)
      ...
      smallworldpathsindex.append(smallworldpathindex)
      smallworldpathsreject.append(smallworldpathreject)
      smallworldpathsdistance.append(smallworldpathdistance)
  ...
  import numpy
  plotlinklengths(randomnets+smallworldnets+kleinbergnets+smallworldnetsindex+smallworldnetsreject+smallworldnetsdistance, "Link lengths", 
  ...
  plotring(locs, smallworldpathsindex, "small world paths by index", 
           filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-meanlen-{}-smallworldbyindex.png".format(
             len(locs), outdegree, int(100*backoffprobability), hash(tuple(locs)), int(numpy.mean(smallworldpathlensindex))))
  plotring(locs, smallworldpathsreject, "small world paths by reject", 
           filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-meanlen-{}-smallworldbyreject.png".format(
             len(locs), outdegree, int(100*backoffprobability), hash(tuple(locs)), int(numpy.mean(smallworldpathlensreject))))
  plotring(locs, smallworldpathsdistance, "small world paths by distance", 
           filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-meanlen-{}-smallworldbydistancne.png".format(
             len(locs), outdegree, int(100*backoffprobability), hash(tuple(locs)), int(numpy.mean(smallworldpathlensdistance))))
