...
#+BEGIN_SRC python :tangle routingsim.py :exports none :results output
  ...
  def smallworldbyreject(locs, start, targets, filepath=None):
      ...
      # def rejectfun(*args):
      #     return False
      pathfold(smallworldnet, locs, numstarts=10, numtargets=20) # , rejectfun=rejectfun)
      # route on small world net
      paths = []
      for target in targets:
        paths.append(greedyrouting(smallworldnet, start, target))
...
#+BEGIN_SRC python :tangle routingsim_plot.py :exports none :results output
    ...
    plotlinklengths(result["random"]["nets"]+result["smallworldapprox"]["nets"]+result["kleinberg"]["nets"], 
                    "Link lengths", 
                    filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-linklengths-random-kleinberg-approx.png".format(
                      len(result["_params"]["locs"]), 
                      result["_params"]["outdegree"], 
                      int(100*result["_params"]["backoffprobability"]), 
                      hash(tuple(result["_params"]["locs"]))),
                    size=result["_params"]["size"])
    plotlinklengths(result["smallworldindex"]["nets"]+result["smallworlddistance"]["nets"]+result["smallworldreject"]["nets"], 
                    "Link lengths", 
    ...
    for name, title in [
      ...
      plotring(result["_params"]["locs"], result[name]["paths"], "random paths", 
