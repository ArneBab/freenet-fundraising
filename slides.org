...
#+BEGIN_SRC python :tangle routingsim.py :exports none :results output
  ...
  def pathfold(net, locs, numstarts=5, numtargets=5, rejectfun=rejectbydistance):
          ...
          for target in targets:
              ...
              for n, node in enumerate(path):
                ...
                if not rejectfun(node, peers, target):
                  peers[random.choice(range(len(peers)))] = target
  ...
  def smallworldbyreject(locs, start, targets, filepath=None):
      ...
      # def rejectfun(*args):
      #     return False
      pathfold(smallworldnet, locs, numstarts=10, numtargets=100) # , rejectfun=rejectfun)
      # route on small world net
      paths = []
      for target in targets:
        paths.append(greedyrouting(smallworldnet, start, target))
  ...
  plotlinklengths(randomnets+smallworldnets+kleinbergnets, "Link lengths", 
           filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-linklengths-random-kleinberg-approx.png".format(
             len(locs), outdegree, int(100*backoffprobability), hash(tuple(locs))))
  plotlinklengths(smallworldnetsindex+smallworldnetsdistance+smallworldnetsreject, "Link lengths", 
           filepath="size-{}-peers-{}-backoffpercentage-{:03}-hash-{}-linklengths-index-distance-reject.png".format(
             len(locs), outdegree, int(100*backoffprobability), hash(tuple(locs))))
