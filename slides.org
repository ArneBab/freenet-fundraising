...
#+BEGIN_SRC python :tangle routingsim.py :exports none :results output
  ...
  def step(path, node, peers, target, maxhtl=100):
    if path[maxhtl:]:
         raise ValueError("Reached the limit of {} steps with path path to {}: {}.".format(maxhtl, target, path))
  ...
  def pathfold(net, locs, numstarts=20, numtargets=3, rejectfun=rejectbydistance):
