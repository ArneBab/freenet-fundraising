...
** TODO Blogging: Flog Helper                                      :noexport:

# Flog helper, Sharelink
...
#+caption: Link length distribution in Freenet. The green line would be an ideal network. Blue would be a random network. Red is the measured distribution. Randomization from local requests, churn and inhomogenity in the network skew the distribution. Average path length with 5-10k nodes: 6
...
- Provides a decentral, anonymous datastore and many applications.
- It scales.
- We want to raise money to make it easy to use.
