...
#+BEGIN_SRC python :tangle routingsim.py :exports none :results output
  ...
  def plotlinklengths(nets, title, filepath=None):
    ...
    for net in nets:
        ...
        pylab.plot(sorted(lengths), numpy.arange(len(lengths))/float(len(lengths)), linewidth=3)
    pylab.xscale("log")
...
#+RESULTS:
: random: [379, 5, 296, 15, 15, 7, 356, 100, 308, 113]
: small world: [8, 11, 12, 11, 8, 8, 8, 10, 8, 7]
